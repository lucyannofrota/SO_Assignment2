...
void free_resources(void){
    ...
    sem_close("serv_c_sem_r");
    sem_close("serv_c_sem_w");
...
int main(int arc, char **argv){
    ...
    int fd = open("s_mem",O_CREAT | O_RDWR | O_TRUNC, 0666);
    if(fd == -1){
        perror("open() error.");
    }
    else{
    ...
    sem_t *sem_r = sem_open("serv_c_sem_r",O_CREAT, 0666, 0);
    sem_t *sem_w = sem_open("serv_c_sem_w",O_CREAT, 0666, 1);
    ...
    char f_number[strlen("-0.")-FLT_MIN_10_EXP+1];
    int i;
